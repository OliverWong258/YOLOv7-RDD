...
class Sobel(nn.Module):
    def __init__(self, use_x, use_y, padding=1, stride=1):
        ...
        weight_x_init=0.5
        weight_y_init=0.5
        self.weight_x_factor = nn.Parameter(torch.tensor(weight_x_init, dtype=torch.float32), requires_grad=True)
        self.weight_y_factor = nn.Parameter(torch.tensor(weight_y_init, dtype=torch.float32), requires_grad=True)

    def forward(self, x):
        # 进行 Sobel 卷积，不做灰度转换
        # x 的形状是 [batch, channels, H, W]
        ...
        # weight_x = self.weight_x.to(device)
        # weight_y = self.weight_y.to(device)
        
        edge_x = F.conv2d(x, self.weight_x.repeat(channels, 1, 1, 1), padding=self.padding, groups=channels, stride=self.stride)
        edge_y = F.conv2d(x, self.weight_y.repeat(channels, 1, 1, 1), padding=self.padding, groups=channels, stride=self.stride)

        # 批量归一化和激活函数
        # self.bn = nn.BatchNorm2d(channels).to(device)
        # self.act = nn.SiLU()

        if self.use_x and self.use_y:
            edge = self.weight_x_factor * edge_x + self.weight_y_factor * edge_y
            return edge # self.act(self.bn(edge))
        elif self.use_x:
            return edge_x # self.act(self.bn(edge_x))
        elif self.use_y:
            return edge_y # self.act(self.bn(edge_y))
        else:
            print("Error in Sobel: must choose a direction")
            sys.exit(1)
# -------------------------Sobel End---------------------------
...
class WeightedSum(nn.Module):
    def __init__(self, num_inputs):
        ...
        # 定义一个可学习的权重向量，大小为num_inputs
        self.weights = nn.Parameter(torch.ones(num_inputs) / num_inputs)  # 初始化为均匀分配的权重
...
# ---------------------------GSConv Begin---------------------------
# 手动将 Conv中的 self.act = nn.Mish()

class GSConv(nn.Module):
    # GSConv https://github.com/AlanLi1997/slim-neck-by-gsconv
    def __init__(self, c1, c2, k=1, s=1, g=1, act=nn.Mish()):
        super().__init__()
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, k, s, None, g, act)
        self.cv2 = Conv(c_, c_, 5, 1, None, c_, act)
...
if __name__ == "__main__":
    input_tensor = torch.randn(1, 3, 256, 256)  # 假设输入是一个 batch_size=1、3个通道、64x64 的图像

    # 测试 Sobel 类输出
    sobel = Sobel(use_x=True, use_y=True)
    sobel_output = sobel(input_tensor)

    # 验证 Sobel 输出的尺寸
    print(f"Sobel 输出尺寸: {sobel_output.shape}")
